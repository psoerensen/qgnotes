...
For this section, we will cover three scenarios, all based on the M-BLUP and G-BLUP models. To solve the conditional expectations, we will be relying on standard probability theory and linear algebra. These tools are briefly described in the `Stat boxes'.
...
To achieve the goal, it is useful to write down the joint distribution of $\vect{b}$, $\vect{g}$ and $\vect{y}$. The starting point to build this joint distribution is to keep in mind the assumptions of M-BLUP and G-BLUP as summarised in table \ref{tab:blup.assumptions} for the general form.
...
\begin{table}[!b]
	...
	\begin{threeparttable}
		...
		\begin{tabular}{ll}
		  \toprule
		  $\vect{y} = \matr{X}\bfbeta + \matr{ZW}\vect{b}+\vect{e}$  & $\E(\vect{y}) = \matr{X}\bfbeta$ \\
...
Here, we will briefly display the MME for G-BLUP and M-BLUP with two random factors.
...
\end{equation}
in the same manner as we did in section \ref{sec:g2b}. The genomic values for individuals in the valdiation set $\vect{a}_2$ and those in the training set as $\vect{a}_1$ are assumed to follow a multivariate normal distribution:
...
\begin{statbox}[floatplacement=!h,label={stat:lin.eq.dist}]{Joint distribution of linear equations}
  ...
  {\bfseries Example:} Using the M-BLUP model, $y^\ast = y$,
  and we can set ${A} = (W ~ I_n)$, ${x} = {\binom{b}{e}}$, ${c} = {1}_n\mu$, and ${m} = {0}$.
  The variance-covariance matrix for $b$ and $e$ is ${V} = \bigl(\begin{smallmatrix} {I}_m \sigma^2_b & {0} \\ {0} & {D}_n \sigma^2_e   \end{smallmatrix} \bigr)$.
