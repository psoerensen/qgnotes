...
\begin{align}
			y=\mu+a+e	 \notag
\end{align}
where  $\mu$ is the population mean, $a$ is the additive genetic effect, and $e$ is the environmental deviation (or residual) not explained by the genetic effects in the model. 
...
\end{align}
where $\sigma_{a_{12}}$ is the genetic covariance and  $\sigma_{a_{1}}^2$ and $\sigma_{a_{2}}^2$ are the respective variances of the additive genetic values for each of the two traits in the population. 
...
__Genetic relationships__ for the individuals in the breeding population is required. Genetic relationships can be inferred from a pedigree or alternatively from genetic markers. Individuals and their parents need to be uniquely identified in the data. Information about birth dates, breed composition, and genotypes for various markers or QTLs could also be stored. If individuals are not uniquely identified, then measuring the genetic change of the population may not be
possible. 
...
For estimating genetic parameters we need to specify a model that describes the genetic and non-genetic factors that may affect the trait phenotypes. Often the non-genetic factors are referred to as systematic effects 
...
Here we make a distinction between fixed effects that determine the level (expected means) of observations, and 
random effects that determine variance. A model exists of at least one fixed (mean) 
and one random effect (residual error variance). If observations are additionally influenced by a 
genetic contribution of the individuals, then a genetic variance component also exists. In 
such a situation, we have two components contributing to the total variance of the 
...
where  $\mu$ is the population mean, $a$ is the additive effect, and $e$ is the environmental deviation (or residual) not explained by the systematic effects or the genetic effects in the model. We assume that the additive genetic effect, $a$, and the residual term, $e$, are normally distributed such that $a \sim N(0,\sigma^2_{a})$ and $e \sim N(0,\sigma^2_{e})$. The goal of the statistical analysis is to derive estimates for the additive genetic variance $\sigma^2_{a}$, and the residual variance $\sigma^2_{e}$. 

The statistical model is a formal representation of our quantitative genetic theory, but it is important to realize that all models are simple approximations of how factors influence a trait. The goal of the statistical analysis is to find the best practical model that explains the most variation in the data. The methods used for estimating genetic parameters is based on statistical concepts such as random variables, multivariate normal theory and linear (mixed) models. These concepts and their use will be explained in the following sections. 
