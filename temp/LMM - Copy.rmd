...
It is important here to understand the distinction between what is known as the 'true model' and an 'instrumental model'. The true model is what generated the data; in the context of these notes, this would be the biological machinery of genes being expressed and that ultimately produce the observed phenotypes. As we are still trying to infer the workings of this complex biological machinery,
we instead refer to an instrumental model that reflects our understanding and what we are trying to compute.

For the following, we refer to the number of genotyped individuals as $q$, number of observations as $n$, and number of markers as $m$. To introduce the two models, we are starting with simplified versions of the models, assuming one observation per individual. This removes the necessity for the ${Z}$ matrix, which will be introduced in chapter \ref{ch:lmm2}. We also assume that fixed effects have been reduced to a single intercept, i.e.\ $X\beta = \mu$.
...
\begin{compactdesc}
  \item[$\vect{y}$] is the $n$-length vector of observations and is the \emph{linear combination} of the \emph{random variables} $\vect{b}$ and $\vect{e}$ plus 
	...
	\item[$\matr{W}$] is the $n \times m$ genotype matrix\footnote{Generally, $\matr{W}$ has $q$ rows, one row for each individual.},
	\item[$\vect{b}$] is the $m$-length vector of marker effects, and
	\item[$\vect{e}$] is the $n$-length residual vector.
\end{compactdesc}

The genotype matrix $\matr{W}$ is the (scaled and centred) marker matrix, linking each of the $q$ individuals to the genotype at each of the $m$ loci.
See section \ref{sec:genotype.matrix} for a discussion of this matrix.
...
Step \ref{eq:mblup.var.3} is possible because the two random variables are uncorrelated.
...
The genotype matrix $\matr{W}$ can be designed in several ways. The overall assumption is that it links the $q$ genotyped individuals to the $m$ markers (after quality control, etc.). It is therefore an $q \times m$ matrix, where each row corresponds to an individual and each column to a marker.
