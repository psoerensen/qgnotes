...
where $\sigma^2_{a}$ is the additive genetic variance, $\sigma^2_{d}$ is the dominance variance, and $\sigma^2_{e}$ is the residual variance. This entails that the observed phenotype ($y$) is also normally distributed $y \sim N( \mu,\sigma^2_{y})$ with the overall phenotypic variance:
...
Heritability and genetic correlations are the key genetic parameters used in estimating an individuals' genetic predisposition to a complex trait. They are defined in terms of the variance components ($\sigma^2_{a}$ and $\sigma^2_{e}$) as presented in the section above.    
...
In modern genetic studies of complex phenotypes, genome-wide association studies (GWAS) are commonly conducted to investigate the underlying genetic basis of the trait in question. From such studies, the SNP-heritability ($h_{SNP}^2$) can be estimated (\textcolor{red}{see section on 'Summary statistics'}), which is the proportion of the total phenotypic variation that can be explain by \textit{common} genetic variants (\textit{i.e.}, typically single nucleotide polymorphisms (SNPs) with a minor allele frequency $>0.01$). In case-control GWAS, heritability estimates might be biased because the scale at which the phenotype is measured at is different from the scale at which the heritability is expressed at. Also, often the proportion of cases is larger than the disease prevalence in the population leading to ascertainment bias. Therefore, the observed genomic heritability ($h_o^2$) can be transformed to the liability scale ($h_l^2$) \citep{Lee2011}:
\begin{equation}
...
\end{align}
Here, we make a distinction between fixed effects, that determine the level (expected mean) of observations, and random effects that determine the variance. A model consists of at least one fixed effect (\textit{i.e.}, the mean) and one random effect (the residual error variance). If observations also are influenced by a genetic effect, then a genetic variance component exists as well^[Multiple genetic variance components may be defined, \textit{e.g.}, by partitioning the total genetic variance by functional groups. See the document \textcolor{red}{Estimation-of-Genomic-Breeding-Values.pdf} for additional details.]. In that situation, we have two components contributing to the total variance of the observations: a genetic and a residual variance component. 

<!--- Out commented section -->
\begin{comment}
...
\begin{align}
 ...
 Var(e) &= I\otimes V_e \notag
\end{align}
where $V_a$ and $V_e$ are square matrcies of genetic and residual (co)variances among traits, respectively. To simplify, we assumed one observation per individual per trait and therefore the $Z$-matrix reduces to an identity matrix which can be left out of the equation system.


## Genetic relationships among individuals 
Estimating narrow sense heritability ($h^2$) using REML (similar to the parent-offspring regression and ANOVA methods) requires that the phenotypic covariance between related individuals can be expressed by their additive genetic relationship ($A$) and the additive genetic variance ($\sigma_a^2$). Related individuals share more alleles and thus resemble each other (have correlated phenotypes, to an extent that depends on additive genetic relationships). 
...
Each off-diagonal elements $(A_{ij})$ is the genetic relationship between individuals $i$ and $j$. Multiplying the matrix $A$ by the additive genetic variance $\sigma_a^2$ leads to the covariance among the individual genetic values. Thus, if $a_i$ is the genetic value of individual $i$ then,
