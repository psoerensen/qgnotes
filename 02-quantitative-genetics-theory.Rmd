...
In practice, the genotype-environment interaction effect can be important for the phenotype of individuals, but for the sake of simplicity we will ignore them in the remainder of this section.

### Genetic effects and genetic values
...
#### Additive Effects
Assume one locus with two alleles, A1, and A2. Assume also that the effect of the
alleles are +1, and -1, respectively. If the genetic effects are entirely additive, then
the value of the possible genotypes would be the sum of their respective allele effects, i.e
\begin{align}
\begin{split}
{A}_1{A}_1 &= 1 + 1 &= +2 \\
{A}_1{A}_2 &= 1 - 1 &=  0 \\
...
#### Dominance Effects
Dominance genetic effects are the interactions among alleles at a given locus. This is
an effect that is extra to the sum of the additive allelic effects. Each genotype would
have its own dominance effect, let these be denoted as $\delta_{ij}$ , and each of them are non-zero
quantities. Using the previous example, the additive and dominance effects would give
\begin{align}
\begin{split}
{A}_1{A}_1 &= 1 + 1 + \delta_{11} &= +2 + \delta_{11} \\
{A}_1{A}_2 &= 1 - 1 + \delta_{12} &= 0 + \delta_{12} \\
{A}_2{A}_2 &= -1 - 1 + \delta_{22} &= -2 + \delta_{22} 
\end{split}
\end{align} 


#### Epistatic Genetic Effects
Epistatic genetic effects encompass all possible interactions among the m loci. This includes all two way interactions, three way interactions, etc. As well, epistasis includes interactions between additive effects at different loci, interactions between additive effects at one locus with dominance effects at a second locus, and interactions between dominance effects at different loci.
